...
def directional(data, d=0.1):
    ...
    link to source: https://www.researchgate.net/publication/311940166_Evolving_Trading_Strategies_Using_Directional_Changes
    '''
    ...
    for i in range(len(data)):
        if event == 'upturn':
            if data['close'][i] <= high * (1 - d):
                ...
                if high < data['close'][i]:
                    high = data['close'][i]
                    
        else: 
            if data['close'][i] <= low * (1 + d):
                event = 'upturn'
                high = data['close'][i]
                data['sig'][i] = data['TWAP'][i]
                data['sign'][i] = "buy"
...
df['signal'] = np.where(directions['sign'] == 'buy', 1, -1)
...
df['TWAP'].plot()
df['signal'].plot(secondary_y='signal')
plt.show()    
